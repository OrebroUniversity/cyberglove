...
void insert(unsigned char into[], int at, unsigned char value,int* used,int size) 
{ 
//insert the artifical sensor values
  memmove (&into[at+1], &into[at], (size-at+(*used))*sizeof(into[0])); 
 into[at]= value; 
 (*used)++;
};
...
//===========rkg-HACK to account for the fact that the AASS Cyberglove only has 18 sensors===
//if (read_stepping(serial_port_fd, (unsigned char*)&ch, GLOVE_SIZE+2)) {
//  error(0,0,"serial port timeout");
//  goto restart;
//};

#ifdef ACTUAL_GLOVE_SIZE==18
  if (read_stepping(serial_port_fd,(unsigned char*)&ch, ACTUAL_GLOVE_SIZE+2)) 
  {
    //error(0,0,"serial port timeout");
    goto restart;
  }
  ...
  {  
    insert((unsigned char*)&ch,15,ch[14],&an,ACTUAL_GLOVE_SIZE); //Replicating pinkie PIJ as pinkie DIJ
    insert((unsigned char*)&ch,12,ch[11],&an,ACTUAL_GLOVE_SIZE); //Replicating ring PIJ as ring DIJ
    insert((unsigned char*)&ch,9,ch[8],&an,ACTUAL_GLOVE_SIZE);   //Replicating middle PIJ as middle DIJ
    insert((unsigned char*)&ch,7,ch[6],&an,ACTUAL_GLOVE_SIZE);   //Replicating index PIJ as index DIJ
  }
#else
  if (read_stepping(serial_port_fd, (unsigned char*)&ch, GLOVE_SIZE+2)) 
  {
